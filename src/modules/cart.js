// Cart Module — LuxCart (with Razorpay checkout)
...
async function handleCheckout() {
  ...
  try {
    ...
    const options = {
      ...
      modal: {
        ondismiss: function () {
          showToast('info', 'Payment Cancelled', 'Checkout closed before completion');
        }
      },
      theme: { color: '#c9a96e' }
    };
    ...
    rzp.on('payment.failed', function (response) {
      const desc = response?.error?.description || response?.error?.reason || 'Please try again';
      showToast('error', 'Payment Failed', desc);
    });
    rzp.open();
  } catch (err) {
